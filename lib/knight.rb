...
def knight_moves(current_step, target, path = [])
  ...
  # p "Current Step:#{current_step}, Current differences X:#{x_axis_difference} Y:#{y_axis_difference}"
  ...
  if x_axis_difference > 0 && y_axis_difference > 0
  ...
  elsif x_axis_difference < 0 && y_axis_difference < 0 || x_axis_difference == 0
    ...
    if x_axis_value + 1 < 7 && y_axis_value + 2 < 7
      knight_moves(move_1_oclock(current_step), target, path)
    elsif x_axis_value - 1 > 0 && y_axis_value + 2 < 7
      knight_moves(move_11_oclock(current_step), target, path)
    end
  elsif y_axis_difference != 0
    if x_axis_value - 1 > 0 && y_axis_value - 2 > 0
      knight_moves(move_7_oclock(current_step), target, path)
    elsif x_axis_value + 1 < 7 && y_axis_value - 2 > 0
      knight_moves(move_5_oclock(current_step), target, path)
    end
  end
  path
